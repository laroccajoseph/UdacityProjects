...
def model_trainer(vgg_model, epochs, data_set, criteron, data_set_valid):
    vgg_model.train()
    running_loss = 0
    accuracy = 0
    for e in range(epochs):
        print(e, "/", epochs)
        running_loss = 0
        for images, labels in iter(data_set):
            ...
            val_loss, accuracy = model_validator(vgg_model, data_set_valid, criteron)
        else:
            print(f"Training Loss: {running_loss/len(data_set)}")
            print(f"Training Validation: {val_loss}")
            print(f"Accuracy: {accuracy}")
            
def model_validator(vgg_model, data_set, criteron):
    vgg_model.eval()
    test_loss = 0
    acc = 0
    for images, labels in iter(data_set):
        ...
        with torch.no_grad():
            vgg_model.to("cpu")
        log_ps = vgg_model(images)
        test_loss += criteron(log_ps,labels)
        ps = torch.exp(log_ps)
        top_p, top_class = ps.topk(1, dim = 1)
        equals = top_class == labels.view(*top_class.shape) 
        acc = torch.mean(equals.type(torch.FloatTensor))
        new_acc = (acc/len(data_set))
        new_loss = (test_loss/len(data_set))
...
model_trainer(model,8, dataloaders, criterion, dataloaders_valid)
